...
const EditFabric = () => {
  ...
  useEffect(() => {
    if (fabricData) {
      reset({
        ...
        shortDescription: fabricData.shortDescription || '',
        description: fabricData.description || '',
        color: fabricData.color || '', // <-- підставляємо колір
  ...
  const onSubmit = async data => {
    ...
    // готуємо завантаження картинок (опційно)
    const formData = new FormData();
    formData.append('folderName', 'fabrics');
    formData.append('fabricId', fabricData.id);
    ...
    try {
      ...
      const updatedFabric = {
        ...
        shortDescription: data.shortDescription,
        description: data.description,
        color: data.color || '', // <-- передаємо колір
        price: data.price || 0,
        suggestedGarments: data.suggestedGarments,
        imageUrls: [
          uploadData.mainImageUrl || fabricData.imageUrls[0],
      ...
      // Скролимо назад до адмін-секції
      setTimeout(() => {
        const el = document.getElementById('admin-fabrics');
        if (el) el.scrollIntoView({ behavior: 'smooth' });
      }, 100);
